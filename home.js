...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ── Personalized Learning Paths (Recommended Next) ─────────────────────
    const recommendedListEl = document.getElementById("recommendedList");
    if (recommendedListEl && window.quizProgress && typeof window.quizProgress.getRecommendedTopics === "function") {
        // Ensure utility exists before usage (in case scripts are reordered)
        ...
        if (recommended.length === 0) {
            recommendedListEl.innerHTML = `<p style="color: var(--text-muted); text-align: center; margin: 12px 0;">No recommendations yet—start a quiz to build your learning path.</p>`;
        } else {
            recommended.forEach((item, idx) => {
                ...
                const weakness = typeof item.weakness === "number" ? item.weakness : null;
                ...
                card.innerHTML = `
                    <div class="assignment-info">
                        <h3 style="margin: 0; font-size: 1.1rem; color: var(--text-color); font-weight: 600;">${(typeof escapeHTML === 'function' ? escapeHTML : window.escapeHTML)(String(topicLabel))}</h3>

                        <div class="assignment-meta">
                            <span class="difficulty-badge" style="background: rgba(255,255,255,0.06); color: var(--accent-color);">#${idx + 1}</span>
                            <span>•</span>
                            <span>${(typeof escapeHTML === 'function' ? escapeHTML : window.escapeHTML)(hint)}</span>
